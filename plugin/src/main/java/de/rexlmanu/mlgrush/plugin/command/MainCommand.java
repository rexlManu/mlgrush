...
public class MainCommand implements CommandExecutor, TabExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    Optional<SubCommand> optional =
        this.commands.stream().filter(s -> s.name().equals(targetName)).findAny();
    optional.ifPresent(
        subCommand ->
    ...
    if (!optional.isPresent()) {
      sender.sendMessage(
          MessageFormat.of(
              Constants.PREFIX + "Es konnte kein Command mit &8'&a%s&8' &7gefunden werden.",
              targetName));
    }
    return true;
  }
                    ...
                    .map(
                        subCommand ->
                            subCommand.suggestions(
                                player, Arrays.copyOfRange(args, 1, args.length)))
                    .findAny()

...
public class StatsCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    PlayerProvider.find(((Player) sender).getUniqueId())
        .ifPresent(
            gamePlayer -> {
              if (args.length == 1) {
                String name = args[0];
                ...
                if (Bukkit.getPlayer(name) != null) {
                  PlayerProvider.find(Bukkit.getPlayer(name).getUniqueId())
                      .ifPresent(
                          target ->
                              this.printStats(
                                  gamePlayer, target.data(), target.player().getName()));
                  return;
                }
                if (this.cooldown.containsKey(gamePlayer.uniqueId())
                    && !UUIDFetcher.isCached(name)) {
                  gamePlayer.sendMessage(
                      "Bitte gedulde dich noch bis du wieder Stats abfragen darfst.");
                  return;
                }
                UUIDFetcher.getUUID(
                    name,
                        ...
                        GameManager.instance()
                            ...
                            .whenComplete(
                                (gamePlayerData, throwable) -> {
                                  this.cooldown.put(
                                      gamePlayer.uniqueId(),
                                      System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(90));
                                  if (throwable != null) {
                                    gamePlayer.sendMessage(
                                        "Dieser Spieler konnte nicht gefunden werden.");
                                    return;
                                  }
        ...
        .whenComplete(
            (rank, throwable) -> {
              ...
              Stream.of(
                      ...
                      String.format(
                          "&8  ▶ &7KD &8● &a%.2f",
                          checkForNan(statistics.kills(), statistics.deaths())),
                      "",
                      String.format(
                      ...
                      String.format(
                          ...
                          checkForNan(statistics.wins(), statistics.games()) * 100),
                      "")

...
public class FileDatabase implements DatabaseContext {
  ...
  public CompletableFuture<GamePlayerData> loadData(UUID uniqueId) {
    return CompletableFuture.supplyAsync(
        () -> this.repository.find(Key.wrap(uniqueId)).orElse(new GamePlayerData(uniqueId)));
  }
  ...
  public CompletableFuture<Integer> getRanking(UUID uniqueId) {
    return CompletableFuture.supplyAsync(
        () -> {
          List<GamePlayerData> playerDataList =
              this.repository.all().stream()
                  .sorted((o1, o2) -> o2.statistics().wins() - o1.statistics().wins())
                  .collect(Collectors.toList());
          GamePlayerData data =
              playerDataList.stream()
                  .filter(gamePlayerData -> gamePlayerData.getKey().equals(uniqueId.toString()))
                  .findAny()
                  .orElse(null);

...
public class DetectionTask implements Runnable {
  public DetectionTask() {
    Bukkit.getScheduler()
        .runTaskTimerAsynchronously(GamePlugin.getProvidingPlugin(GamePlugin.class), this, 0, 20);
  }
  ...
  public void run() {
    PlayerProvider.PLAYERS.forEach(
        gamePlayer -> {
          ...
          detection.clickAverageSecondly(
              detection.clickHistory().stream()
                  .mapToDouble(Integer::doubleValue)
                  .average()
                  .orElse(0));
          if (detection.clickHistory().size() >= 3) {
            detection.clickAverage(
                detection.clickHistory().stream()
                    .mapToDouble(Integer::doubleValue)
                    .average()
                    .getAsDouble());
          ...
          detection.placeAverageSecondly(
              detection.placeHistory().stream()
                  .mapToDouble(Integer::doubleValue)
                  .average()
                  .orElse(0));
          if (detection.placeHistory().size() >= 3) {
            detection.placeAverage(
                detection.placeHistory().stream()
                    .mapToDouble(Integer::doubleValue)
                    .average()
                    .getAsDouble());

...
public class ArenaConfigurationInventory implements Listener {
  ...
  private static final ItemStack FINISH =
      ItemStackBuilder.of(Material.LIME_DYE).name("&aAnfrage senden").build();
  ...
  private OptionItem<Integer> maximalPointsOption =
      this.register(
          new IntegerOptionItem(
              ItemStackBuilder.of(Material.RED_BED)
                  .name("&8» &aPunkte")
                  .lore("&7Wie viele &aPunkte&7 braucht", "&7man um das Spiel zu-", "&7gewinnen.")
                  .build(),
              20,
              10,
              30,
              1));
  private OptionItem<Boolean> nohitdelayOption =
      this.register(
          new BooleanOptionItem(
              ItemStackBuilder.of(Material.ARROW).name("&8» &aNoHitDelay").build(), 21, false));
  private OptionItem<Boolean> blockBreakOption =
      this.register(
          new BooleanOptionItem(
              ItemStackBuilder.of(Material.IRON_PICKAXE)
                  .name("&8» &aAuto. Block-Entferner")
                  .lore(
                      "&7Platzierte Blöcke werden nach", "&7einigen Sekunden wieder &aentfernt&7.")
                  .build(),
              22,
              false));
  private OptionItem<Integer> buildHeightOption =
      this.register(
          new IntegerOptionItem(
              ...
              250,
              4));
  private OptionItem<Boolean> knockbackOnlyHeightOption =
      this.register(
          new BooleanOptionItem(
              ItemStackBuilder.of(Material.LEATHER_BOOTS)
                  .color(Color.YELLOW)
                  .name("&8» &aKnockback nur nach Oben")
                  .lore("&7Nur noch Knockback nach", "&aoben &7wird möglich sein.")
                  .build(),
              24,
              false));
  private OptionItem<Boolean> unlimitedBlocksOption =
      this.register(
          new BooleanOptionItem(
              ItemStackBuilder.of(Material.BEDROCK).name("&8» &aUnlimiterte Blöcke").build(),
              12,
              false));
  private OptionItem<Boolean> fallDamageOption =
      this.register(
          new BooleanOptionItem(
              ItemStackBuilder.of(Material.DIAMOND_BOOTS).name("&8» &aFallschaden").build(),
              13,
              false));
  private OptionItem<Boolean> showCpsOption =
      this.register(
          new BooleanOptionItem(
              ItemStackBuilder.of(Material.WOODEN_SWORD)
                  .lore(
                      "&8» &7Du siehst wie viele",
                      "&aKlicks per Sekunde &7dein",
                      "&7Gegner aktuell hat.")
                  .name("&aCPS anzeigen")
                  .build(),
              14,
              false));
  private OptionItem<Integer> spawnProtectionOption =
      this.register(
          new IntegerOptionItem(
              ItemStackBuilder.of(Material.BEACON)
                  .name("&8» &aSpawnProtection")
                  .lore("&7In der SpawnProtection", "&7kann man &akeine&7", "&7Blöcke platzieren.")
                  .build(),
              31,
              3,
              3,
  ...
  public ArenaConfigurationInventory(GamePlayer owner, GamePlayer target) {
    ...
    this.inventory =
        Bukkit.createInventory(
            null, 5 * 9, MessageFormat.replaceColors("&8● &aSpiel konfigurieren"));

    Bukkit.getPluginManager().registerEvents(this, GamePlugin.getProvidingPlugin(GamePlugin.class));
    this.createPattern();
    this.optionItems.forEach(
        optionItem -> this.inventory.setItem(optionItem.slot(), optionItem.itemStack()));
  ...
  private void sendRequest() {
    ArenaConfiguration.ArenaConfigurationBuilder configuration =
        ArenaManager.DEFAULT_CONFIGURATION
            .get()
            .maximalPoints(this.maximalPointsOption.value())
    ...
    target.sendMessage(
        String.format(
            "Du wurdest von &a%s&7 zu einem eigenen Spiel gefordert mit &afolgenden &7Einstellungen:",
            this.owner.player().getName()));
    Stream.of(
            "",
            String.format(
                "  &8▶ &7NoHitDelay &8● &a%s",
                this.nohitdelayOption.value() ? "&aaktiv" : "&7deaktiviert"),
            String.format(
                "  &8▶ &7Knockback nur Oben &8● &a%s",
                this.knockbackOnlyHeightOption.value() ? "&aaktiv" : "&7deaktiviert"),
            String.format(
                "  &8▶ &7Auto. Block-Entferner &8● &a%s",
                this.blockBreakOption.value() ? "&aaktiv" : "&7deaktiviert"),
            ...
            String.format(
                "  &8▶ &7Benötigte Siegespunkte &8● &a%s", this.maximalPointsOption.value()),
            String.format(
                "  &8▶ &7Fallschaden &8● &a%s",
                this.fallDamageOption.value() ? "&aaktiv" : "&7deaktiviert"),
            String.format(
                "  &8▶ &7Unlimiterte Blöcke &8● &a%s",
                this.unlimitedBlocksOption.value() ? "&aaktiv" : "&7deaktiviert"),
            String.format(
                "  &8▶ &7CPS anzeigen &8● &a%s",
                this.showCpsOption.value() ? "&aaktiv" : "&7deaktiviert"),
            "")
        .map(MessageFormat::replaceColors)
        .forEach(s -> target.player().sendMessage(s));
    this.owner.sendMessage(
        String.format(
            "Du hast &a%s&7 zu einem eigenen Spiel gefordert.", target.player().getName()));
  ...
  public void handle(PlayerIngameEvent event) {
    ...
    this.owner.sendMessage(
        String.format(
            "Die Erstellung des Spiels wurde abgebrochen weil &a%s &7in ein Spiel gegangen ist.",
  ...
  public void handle(PlayerQuitEvent event) {
    ...
    this.owner.sendMessage(
        String.format(
            "Die Erstellung des Spiels wurde abgebrochen weil &a%s &7das Spiel verlassen hat.",
            this.target.player().getName()));
    this.owner.sound(Sound.BLOCK_ANVIL_LAND, 2f);
  }

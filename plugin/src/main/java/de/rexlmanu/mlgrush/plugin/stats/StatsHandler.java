        ...
        .whenComplete(
            (gamePlayerData, throwable) -> {
              ...
              for (int i = 0; i < 5; i++) {
                ...
                entries.add(
                    new StatsWallEntry(
                        rank, name, data.statistics().kills(), data.statistics().wins(), uniqueId));
              }
                      ...
                      () -> {
                        entries.forEach(
                            entry -> {
                              GameManager.instance()
                                  .locationProvider()
                                  .get("ranking-" + entry.rank())
                                  .ifPresent(
                                      location -> {
                                        if (!(location.getBlock().getState()
                                        ...
                                        if (sign.getLocation().add(0, 1, 0).getBlock().getState()
                                            instanceof Skull skull) {
                                          if (entry.uniqueId() == null) {
                                            skull.setOwner("MHF_QUESTION");
                                          } else {
                                            OfflinePlayer offlinePlayer =
                                                Bukkit.getOfflinePlayer(entry.uniqueId());

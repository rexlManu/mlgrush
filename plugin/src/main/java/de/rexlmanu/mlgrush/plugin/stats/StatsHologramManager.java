...
public class StatsHologramManager {
  ...
  private CompletableFuture<VirtualHologram> create(GamePlayer gamePlayer) {
    ...
    return GameManager.instance().databaseContext().getRanking(gamePlayer.uniqueId()).thenApply((rank) -> {
      VirtualHologram hologram = new VirtualHologram(this.location, Stream.of(
        "",

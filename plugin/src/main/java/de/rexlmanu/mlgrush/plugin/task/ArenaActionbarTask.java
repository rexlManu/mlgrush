        ...
        .forEach(
            arena -> {
              arena
                  ...
                  .forEach(
                      gamePlayer ->
                          GameManager.instance().scoreboardHandler().updateSidebar(gamePlayer));
              Stream.concat(arena.players().stream(), arena.spectators().stream())
                        ...
                        for (int i = 0; i < arena.gameTeams().size(); i++) {
                          ...
                          if (halfTeamCount < (i + 1)) {
                            ...
                            if (gamePlayer.inspectionMode()) {
                              suffix
                                  .append(" &8● &a")
                                  .append(this.format(team.members().get(0).detection()));
                            }
                          } else {
                            if (gamePlayer.inspectionMode()) {
                              prefix

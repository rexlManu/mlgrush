...
public class ArenaBlockRemoveTask implements Runnable {
  ...
  public void run() {
    ...
    GameManager.instance().arenaManager().arenaContainer().activeArenas().stream()
        .filter(arena -> arena.configuration().autoBlockBreak())
        .forEach(
            arena -> {
              arena
                  .placedBlocks()
                  .forEach(
                      block -> {
                        float progress =
                            Math.min(1.0f, this.blockAnimationMap.getOrDefault(block, 0.0f) + 0.1f);
                        this.blockAnimationMap.put(block, progress);

                        arena.players().stream()
                            .map(GamePlayer::player)
                            .forEach(player -> this.sendBlockAnimation(player, block, progress));
  ...
  private void sendBlockAnimation(Player player, Block block, float progress) {
    ...
    WrapperPlayServerBlockBreakAnimation packet =
        new WrapperPlayServerBlockBreakAnimation(
            block.hashCode(), new Vector3i(block.getX(), block.getY(), block.getZ()), stage);

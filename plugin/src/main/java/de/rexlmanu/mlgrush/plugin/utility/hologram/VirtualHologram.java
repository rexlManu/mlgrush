...
public class VirtualHologram {
  ...
  private void spawnFor(Player player) {
    this.lineEntities.forEach(line -> {
      ...
      PacketEvents.getAPI().getPlayerManager().sendPacket(player, new WrapperPlayServerEntityMetadata(line.entityId(), this.metadata(line.text())));
    });
  }
  ...
  private List<EntityData<?>> metadata(String text) {
    ...
    metadata.add(new EntityData<>(2, EntityDataTypes.OPTIONAL_ADV_COMPONENT, Optional.of(this.component(text))));
  ...
  private void rebuild() {
    ...
    for (String line : this.lines) {
      ...
      this.lineEntities.add(new LineEntity(
        ENTITY_IDS.getAndIncrement(),
        UUID.randomUUID(),
        new Location(this.location.getWorld(), this.location.getX(), y, this.location.getZ(), this.location.getYaw(), this.location.getPitch()),
        line
      ));
    }
  }
